    ...
    if (!ret)               /* btree includes key */
    {
        ...
        if (preceding->num >= M) /* case 2a */
        {
            /*
               ...
               then find the predecessor k¡ä of k in the subtree rooted at y. 
               Recursively delete k¡ä, and replace k by k¡ä in x. 
               (Finding k¡ä and deleting it can be performed in a single downward pass.)
             */
        ...
        if (successor->num >= M)  /* case 2b */
        {
            /*
               ...
               has at least t keys, then find the successor k¡ä of k 
               in the subtree rooted at z. Recursively delete k¡ä, and 
               replace k by k¡ä in x. (Finding k¡ä and deleting it can 
